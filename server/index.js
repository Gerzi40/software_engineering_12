...
app.get("/schedule/renter-id/:renterId", (req, res) => {
    ...
    db.query("SELECT courtId, scheduleDate, scheduleTypeId, renterId, userName, scheduleRating, isRated,  FROM schedule JOIN court ON schedule.courtId = court.courtId JOIN schedule_type ON schedule.scheduleTypeId = schedule_type.scheduleTypeId JOIN court_type ON court.courtTypeId = court_type.courtTypeId WHERE renterId = ?", [renterId], (err, result) => {
        if(err){
            res.send({message: err})
        } else {
            res.send(result)
